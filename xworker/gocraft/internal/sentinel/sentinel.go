// Package sentinel provides sentinel support to gocraft.
package sentinel
...
type Sentinel struct {
	...
	// Master address to detect master switch and emit event
	masterAddress string

	failoverCallback []FailOverCallback
}

type FailOverCallback func(oldMasterAddr, newMasterAddr string)
...
func (s *Sentinel) MasterAddr() (string, error) {
	...
	newAddress, _ := res.(string)

	s.mu.Lock()
	if s.hasMasterChanged(newAddress) {
		s.execFailoverCallback(s.masterAddress, newAddress)
	}

	s.masterAddress = newAddress
	s.mu.Unlock()

	return newAddress, nil
}

func (s *Sentinel) hasMasterChanged(newAddress string) bool {
	return s.masterAddress != "" && s.masterAddress != newAddress
}
...
func (s *Sentinel) execFailoverCallback(oldMasterAddr string, newMasterAddr string) {
	for _, callbackFunc := range s.failoverCallback {
		...
		go func() {
			...
			callback(oldMasterAddr, newMasterAddr)
		}()
	}
}
...
func (s *Sentinel) recover() {
	if err := recover(); err != nil {
		fmt.Printf(
			"SentinelWrapper: Recovered from panic in sentinel failoverCallback at timestamp %s from error: %v",
...
func (s *Sentinel) RegisterFailoverCallback(callback FailOverCallback) {
	s.mu.Lock()
	s.failoverCallback = append(s.failoverCallback, callback)
	s.mu.Unlock()
}
